...
// File autogenerated at 2026-02-17 15:40:03 CET with M2C version 1.0.0
...
bool PlaceObjectSkill::start(int argc, char*argv[])
{
  ...
  m_actionClient = rclcpp_action::create_client<overarching_msgs::action::place_object>(m_node, "/PlaceComponet/place_object");
  ...
  m_stateMachine.connectToEvent("PlaceComponet.place_object.SendGoal", [this]([[maybe_unused]]const QScxmlEvent & event){
    RCLCPP_INFO(m_node->get_logger(), "PlaceObjectSkill::PlaceComponet.place_object.SendGoal");
    RCLCPP_INFO(m_node->get_logger(), "calling send goal");
    std::shared_ptr<rclcpp::Node> nodeplace_object = rclcpp::Node::make_shared(m_name + "SkillNodeplace_object");
    rclcpp_action::Client<overarching_msgs::action::place_object>::SharedPtr clientplace_object  =
    rclcpp_action::create_client<overarching_msgs::action::place_object>(nodeplace_object, "/PlaceComponet/place_object");
    overarching_msgs::action::place_object::Goal goal_msg;
    
    send_goal(goal_msg);
    RCLCPP_INFO(m_node->get_logger(), "done send goal");
  });
  m_stateMachine.connectToEvent("PlaceComponet.place_object.ResultRequest", [this]([[maybe_unused]]const QScxmlEvent & event){
      RCLCPP_INFO(m_node->get_logger(), "PlaceObjectSkill::PlaceComponet.place_object.ResultRequest");
      std::shared_ptr<rclcpp::Node> nodeplace_object = rclcpp::Node::make_shared(m_name + "SkillNodeplace_object");
      rclcpp_action::Client<overarching_msgs::action::place_object>::SharedPtr clientplace_object  =
        rclcpp_action::create_client<overarching_msgs::action::place_object>(nodeplace_object, "/PlaceComponet/GoToPoi");
...
void PlaceObjectSkill::send_goal(overarching_msgs::action::place_object::Goal goal_msg)
{
  ...
  while (!m_actionClient->wait_for_action_server(std::chrono::seconds(1))) {
    if (!rclcpp::ok()) {
      RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Interrupted while waiting for the service 'place_object'. Exiting.");
      wait_succeded = false;
      break;
    } 
    retries++;
    if(retries == SERVICE_TIMEOUT) {
      RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Timed out while waiting for the service 'place_object'.");
      wait_succeded = false;
      QVariantMap data;
      data.insert("call_succeeded", false);
      m_stateMachine.submitEvent("PlaceComponet.place_object.GoalResponse", data);
      break;
    }
  }
  if (wait_succeded) {
...
void PlaceObjectSkill::result_callback(const  rclcpp_action::ClientGoalHandle<overarching_msgs::action::place_object>::WrappedResult & result)
{
  switch (result.code) {
  ...
  m_stateMachine.submitEvent("PlaceComponet.place_object.ResultResponse", data);
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "PlaceComponet.place_object.ResultResponse");
}
void PlaceObjectSkill::goal_response_callback(const rclcpp_action::ClientGoalHandle<overarching_msgs::action::place_object>::SharedPtr & goal_handle)
{
  ...
  if (!goal_handle) {
    data.insert("call_succeeded", false);
    m_stateMachine.submitEvent("PlaceComponet.place_object.GoalResponse", data);
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "PlaceComponet.place_object.GoalResponse Failure");
    RCLCPP_ERROR(m_node->get_logger(), "Goal was rejected by server");
  } else {
    data.insert("call_succeeded", true);
    m_stateMachine.submitEvent("PlaceComponet.place_object.GoalResponse", data);
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "PlaceComponet.place_object.GoalResponse Success");
