// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from timer_interfaces_dummy:srv/IsTimerActive.idl
// generated code does not contain a copyright notice
#include "timer_interfaces_dummy/srv/detail/is_timer_active__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

bool
timer_interfaces_dummy__srv__IsTimerActive_Request__init(timer_interfaces_dummy__srv__IsTimerActive_Request * msg)
{
  if (!msg) {
    return false;
  }
  // structure_needs_at_least_one_member
  return true;
}

void
timer_interfaces_dummy__srv__IsTimerActive_Request__fini(timer_interfaces_dummy__srv__IsTimerActive_Request * msg)
{
  if (!msg) {
    return;
  }
  // structure_needs_at_least_one_member
}

bool
timer_interfaces_dummy__srv__IsTimerActive_Request__are_equal(const timer_interfaces_dummy__srv__IsTimerActive_Request * lhs, const timer_interfaces_dummy__srv__IsTimerActive_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // structure_needs_at_least_one_member
  if (lhs->structure_needs_at_least_one_member != rhs->structure_needs_at_least_one_member) {
    return false;
  }
  return true;
}

bool
timer_interfaces_dummy__srv__IsTimerActive_Request__copy(
  const timer_interfaces_dummy__srv__IsTimerActive_Request * input,
  timer_interfaces_dummy__srv__IsTimerActive_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // structure_needs_at_least_one_member
  output->structure_needs_at_least_one_member = input->structure_needs_at_least_one_member;
  return true;
}

timer_interfaces_dummy__srv__IsTimerActive_Request *
timer_interfaces_dummy__srv__IsTimerActive_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  timer_interfaces_dummy__srv__IsTimerActive_Request * msg = (timer_interfaces_dummy__srv__IsTimerActive_Request *)allocator.allocate(sizeof(timer_interfaces_dummy__srv__IsTimerActive_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(timer_interfaces_dummy__srv__IsTimerActive_Request));
  bool success = timer_interfaces_dummy__srv__IsTimerActive_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
timer_interfaces_dummy__srv__IsTimerActive_Request__destroy(timer_interfaces_dummy__srv__IsTimerActive_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    timer_interfaces_dummy__srv__IsTimerActive_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence__init(timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  timer_interfaces_dummy__srv__IsTimerActive_Request * data = NULL;

  if (size) {
    data = (timer_interfaces_dummy__srv__IsTimerActive_Request *)allocator.zero_allocate(size, sizeof(timer_interfaces_dummy__srv__IsTimerActive_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = timer_interfaces_dummy__srv__IsTimerActive_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        timer_interfaces_dummy__srv__IsTimerActive_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence__fini(timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      timer_interfaces_dummy__srv__IsTimerActive_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence *
timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence * array = (timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence *)allocator.allocate(sizeof(timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence__destroy(timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence__are_equal(const timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence * lhs, const timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!timer_interfaces_dummy__srv__IsTimerActive_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence__copy(
  const timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence * input,
  timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(timer_interfaces_dummy__srv__IsTimerActive_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    timer_interfaces_dummy__srv__IsTimerActive_Request * data =
      (timer_interfaces_dummy__srv__IsTimerActive_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!timer_interfaces_dummy__srv__IsTimerActive_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          timer_interfaces_dummy__srv__IsTimerActive_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!timer_interfaces_dummy__srv__IsTimerActive_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
timer_interfaces_dummy__srv__IsTimerActive_Response__init(timer_interfaces_dummy__srv__IsTimerActive_Response * msg)
{
  if (!msg) {
    return false;
  }
  // is_ok
  // is_active
  return true;
}

void
timer_interfaces_dummy__srv__IsTimerActive_Response__fini(timer_interfaces_dummy__srv__IsTimerActive_Response * msg)
{
  if (!msg) {
    return;
  }
  // is_ok
  // is_active
}

bool
timer_interfaces_dummy__srv__IsTimerActive_Response__are_equal(const timer_interfaces_dummy__srv__IsTimerActive_Response * lhs, const timer_interfaces_dummy__srv__IsTimerActive_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // is_ok
  if (lhs->is_ok != rhs->is_ok) {
    return false;
  }
  // is_active
  if (lhs->is_active != rhs->is_active) {
    return false;
  }
  return true;
}

bool
timer_interfaces_dummy__srv__IsTimerActive_Response__copy(
  const timer_interfaces_dummy__srv__IsTimerActive_Response * input,
  timer_interfaces_dummy__srv__IsTimerActive_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // is_ok
  output->is_ok = input->is_ok;
  // is_active
  output->is_active = input->is_active;
  return true;
}

timer_interfaces_dummy__srv__IsTimerActive_Response *
timer_interfaces_dummy__srv__IsTimerActive_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  timer_interfaces_dummy__srv__IsTimerActive_Response * msg = (timer_interfaces_dummy__srv__IsTimerActive_Response *)allocator.allocate(sizeof(timer_interfaces_dummy__srv__IsTimerActive_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(timer_interfaces_dummy__srv__IsTimerActive_Response));
  bool success = timer_interfaces_dummy__srv__IsTimerActive_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
timer_interfaces_dummy__srv__IsTimerActive_Response__destroy(timer_interfaces_dummy__srv__IsTimerActive_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    timer_interfaces_dummy__srv__IsTimerActive_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence__init(timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  timer_interfaces_dummy__srv__IsTimerActive_Response * data = NULL;

  if (size) {
    data = (timer_interfaces_dummy__srv__IsTimerActive_Response *)allocator.zero_allocate(size, sizeof(timer_interfaces_dummy__srv__IsTimerActive_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = timer_interfaces_dummy__srv__IsTimerActive_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        timer_interfaces_dummy__srv__IsTimerActive_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence__fini(timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      timer_interfaces_dummy__srv__IsTimerActive_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence *
timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence * array = (timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence *)allocator.allocate(sizeof(timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence__destroy(timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence__are_equal(const timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence * lhs, const timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!timer_interfaces_dummy__srv__IsTimerActive_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence__copy(
  const timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence * input,
  timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(timer_interfaces_dummy__srv__IsTimerActive_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    timer_interfaces_dummy__srv__IsTimerActive_Response * data =
      (timer_interfaces_dummy__srv__IsTimerActive_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!timer_interfaces_dummy__srv__IsTimerActive_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          timer_interfaces_dummy__srv__IsTimerActive_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!timer_interfaces_dummy__srv__IsTimerActive_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `info`
#include "service_msgs/msg/detail/service_event_info__functions.h"
// Member `request`
// Member `response`
// already included above
// #include "timer_interfaces_dummy/srv/detail/is_timer_active__functions.h"

bool
timer_interfaces_dummy__srv__IsTimerActive_Event__init(timer_interfaces_dummy__srv__IsTimerActive_Event * msg)
{
  if (!msg) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__init(&msg->info)) {
    timer_interfaces_dummy__srv__IsTimerActive_Event__fini(msg);
    return false;
  }
  // request
  if (!timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence__init(&msg->request, 0)) {
    timer_interfaces_dummy__srv__IsTimerActive_Event__fini(msg);
    return false;
  }
  // response
  if (!timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence__init(&msg->response, 0)) {
    timer_interfaces_dummy__srv__IsTimerActive_Event__fini(msg);
    return false;
  }
  return true;
}

void
timer_interfaces_dummy__srv__IsTimerActive_Event__fini(timer_interfaces_dummy__srv__IsTimerActive_Event * msg)
{
  if (!msg) {
    return;
  }
  // info
  service_msgs__msg__ServiceEventInfo__fini(&msg->info);
  // request
  timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence__fini(&msg->request);
  // response
  timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence__fini(&msg->response);
}

bool
timer_interfaces_dummy__srv__IsTimerActive_Event__are_equal(const timer_interfaces_dummy__srv__IsTimerActive_Event * lhs, const timer_interfaces_dummy__srv__IsTimerActive_Event * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__are_equal(
      &(lhs->info), &(rhs->info)))
  {
    return false;
  }
  // request
  if (!timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence__are_equal(
      &(lhs->request), &(rhs->request)))
  {
    return false;
  }
  // response
  if (!timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence__are_equal(
      &(lhs->response), &(rhs->response)))
  {
    return false;
  }
  return true;
}

bool
timer_interfaces_dummy__srv__IsTimerActive_Event__copy(
  const timer_interfaces_dummy__srv__IsTimerActive_Event * input,
  timer_interfaces_dummy__srv__IsTimerActive_Event * output)
{
  if (!input || !output) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__copy(
      &(input->info), &(output->info)))
  {
    return false;
  }
  // request
  if (!timer_interfaces_dummy__srv__IsTimerActive_Request__Sequence__copy(
      &(input->request), &(output->request)))
  {
    return false;
  }
  // response
  if (!timer_interfaces_dummy__srv__IsTimerActive_Response__Sequence__copy(
      &(input->response), &(output->response)))
  {
    return false;
  }
  return true;
}

timer_interfaces_dummy__srv__IsTimerActive_Event *
timer_interfaces_dummy__srv__IsTimerActive_Event__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  timer_interfaces_dummy__srv__IsTimerActive_Event * msg = (timer_interfaces_dummy__srv__IsTimerActive_Event *)allocator.allocate(sizeof(timer_interfaces_dummy__srv__IsTimerActive_Event), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(timer_interfaces_dummy__srv__IsTimerActive_Event));
  bool success = timer_interfaces_dummy__srv__IsTimerActive_Event__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
timer_interfaces_dummy__srv__IsTimerActive_Event__destroy(timer_interfaces_dummy__srv__IsTimerActive_Event * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    timer_interfaces_dummy__srv__IsTimerActive_Event__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence__init(timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  timer_interfaces_dummy__srv__IsTimerActive_Event * data = NULL;

  if (size) {
    data = (timer_interfaces_dummy__srv__IsTimerActive_Event *)allocator.zero_allocate(size, sizeof(timer_interfaces_dummy__srv__IsTimerActive_Event), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = timer_interfaces_dummy__srv__IsTimerActive_Event__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        timer_interfaces_dummy__srv__IsTimerActive_Event__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence__fini(timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      timer_interfaces_dummy__srv__IsTimerActive_Event__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence *
timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence * array = (timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence *)allocator.allocate(sizeof(timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence__destroy(timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence__are_equal(const timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence * lhs, const timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!timer_interfaces_dummy__srv__IsTimerActive_Event__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence__copy(
  const timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence * input,
  timer_interfaces_dummy__srv__IsTimerActive_Event__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(timer_interfaces_dummy__srv__IsTimerActive_Event);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    timer_interfaces_dummy__srv__IsTimerActive_Event * data =
      (timer_interfaces_dummy__srv__IsTimerActive_Event *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!timer_interfaces_dummy__srv__IsTimerActive_Event__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          timer_interfaces_dummy__srv__IsTimerActive_Event__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!timer_interfaces_dummy__srv__IsTimerActive_Event__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
